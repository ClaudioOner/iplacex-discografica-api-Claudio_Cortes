...
public class ArtistaController {
    ...
    @Autowired
    private IArtistaRepository artistaRepo; // Cambiar a IArtistaRepository
    ...
    public ResponseEntity<Artista> HandleInsertArtistaRequest(@RequestBody Artista artista) {
        Artista temp = artistaRepo.save(artista); // Usar save en lugar de insert
        return new ResponseEntity<>(temp, null, HttpStatus.CREATED);
    }

    // Listar objetos de BD según ID
    @SuppressWarnings("null")
    @GetMapping(
        value = "/artista/{id}",
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    public ResponseEntity<Artista> HandleGetAristasRequest(@PathVariable("id") String id) {
    ...
    // Obtener todos los registros desde la BD
    @GetMapping(
        value = "/artistas",
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    ...
    public ResponseEntity<Artista> HandleUpdateArtistaRequest(
        ...
        artista.setId(id); // Cambia esto según cómo tengas definido el ID en la clase Artista
        Artista temp = artistaRepo.save(artista);
        return new ResponseEntity<>(temp, null, HttpStatus.OK);
    }

    // Eliminar registro en base a ID
    @SuppressWarnings("null")
